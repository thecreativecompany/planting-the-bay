...
The `/studio` route now lazy-loads `next-sanity/studio` on the client through `app/studio/[[...tool]]/StudioClient.tsx`. This prevents the Studio from being prerendered during `next build`, which can cause Vercel build failures in the `NextStudio` stack.
